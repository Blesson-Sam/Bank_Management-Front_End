  ...
  private initNavigationScroll() {
    const handleNavScroll = () => {
      ...
      if (productsSection && navBar) {
        ...
        if (productsSectionTop <= 100) {
          // Products section reached, make nav white
          ...
          const navLinks = navBar.querySelectorAll('.nav-link, .auth-link, .logo-text');
          navLinks.forEach((link: any) => {
            link.style.color = '#333333';
          });
        } else {
          // Still in hero section, keep nav transparent/dark
          navBar.style.background = 'var(--background-color)';
          navBar.style.backdropFilter = 'none';
          navBar.style.borderBottom = 'none';
          
          // Reset text colors to light
          const navLinks = navBar.querySelectorAll('.nav-link, .auth-link, .logo-text');
          navLinks.forEach((link: any) => {
            link.style.color = '';
          });
    ...
    this.scrollListener = () => {
      ...
      featureCards.forEach((card: HTMLElement, index: number) => {
        ...
        if (scrollPercent > 0 && scrollPercent < 1) {
          const parallaxValue = (scrollPercent - 0.5) * 20;
          card.style.transform = `translateY(${parallaxValue}px) perspective(1000px) rotateX(${parallaxValue * 0.2}deg)`;
        }
      });

      // Hero title parallax
      const heroTitle = this.elementRef.nativeElement.querySelector('.hero-title');
      if (heroTitle) {
